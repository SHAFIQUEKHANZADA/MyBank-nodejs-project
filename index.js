...
class myBank {
    customers = [];
    async createAcc() {
        const { firstName, lastName, age, gender, accountNumber, balance } = await inquirer.prompt([
            {
                type: "input",
                name: "firstName",
        ...
        console.log(chalk.bold.italic.yellowBright(`congratulations, Mr/s ${cus.FirstName} ${cus.LastName} your account has been created successfully.`));
    }
    async details() {
        ...
        if (cus) {
            console.log(chalk.bold.greenBright.underline(`Account Details:
    ...
    async debit() {
        ...
        if (cus) {
            if (cus.Balance >= parseFloat(amount)) {
                ...
                console.log(chalk.bold.italic.greenBright(`Debited ${amount} from account ${accountNumber}. New balance: ${cus.Balance}`));
            }
            else {
    ...
    async credit() {
        ...
        if (cus) {
            cus.Balance += parseFloat(amount);
            console.log(chalk.bold.italic.greenBright(`Credited ${amount} to account ${accountNumber}. New balance: ${cus.Balance}`));
        }
        else {
